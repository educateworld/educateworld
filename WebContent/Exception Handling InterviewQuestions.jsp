<p><h4>What is Exception in Java?</h4></p>
<p>Exception is an error event.Error can occur during the execution of a program.It disrupts Program’s normal flow.
<p>Exception can arise in different ways and in situations such as software related errors like user may entered wrong data ,network connection failure, hardware failure etc.

<p>while executng java code that time if you get error then an exception object is created and Java Runtime Environment finds the appropriate exception handler. That error handled by JRE and it will continue the program's flow.
 If that exception is not handle to execution then JRE send error to runtime. That means suitable exeception handler is not  to handle the exception. If suitable exception handler is found then the exception object is passed to the handler code to process the exception, known as catching the exception. If no handler is found then application throws the exception to runtime environment and JRE terminates the program.
...
exception handling in java interview questions, exception handling interview questions, java exception interview questions
...
We have to remove the assignment of “e” to new exception object to solve this error.1